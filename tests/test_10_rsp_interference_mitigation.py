...
def test_interference_mitigation():
    ...
    energy_out_peak = np.delete(fft_mag, [tone_peak-1, tone_peak, tone_peak+1]).sum()
    ...
    fft_mag_interfered = np.abs(np.fft.fft(adc_values_interfered)[:adc_reference.shape[0]//2])
    energy_interference = np.delete(fft_mag_interfered, [tone_peak-1, tone_peak, tone_peak+1]).sum()
    ...
    interference_range = np.arange(min(interference_idx),max(interference_idx),1)
    ...
    energy_zeroed = np.delete(fft_mag_zerod, [tone_peak-1, tone_peak, tone_peak+1]).sum()
    ...
    ideal_fft[top_tone_in_zeroed-1:top_tone_in_zeroed+2] = np.fft.fft(adc_interferences_zeroed)[tone_peak-1: tone_peak+2]  
    # ideal_fft[top_tone_in_zeroed-1:top_tone_in_zeroed+2] = fft_reference[tone_peak-1: tone_peak+2]  
    adc_interfered_reconstructed[interference_range] = np.fft.irfft(ideal_fft)[interference_range]

    fft_mag_reconstructed = np.abs(np.fft.fft(adc_interfered_reconstructed)[:adc_reference.shape[0]//2])
    """plot, axs = plt.subplots(nrows=1, ncols=2, figsize=(10, 4))
    axs[0].plot(fft_mag, 'r-')
    axs[0].plot(fft_mag_interfered, 'g-')
    axs[0].plot(fft_mag_zerod,'b-')
    axs[0].plot(fft_mag_reconstructed, 'black')
    axs[0].set_title(f"{top_tone_in_zeroed}")"""
    energy_reconstructed = np.delete(fft_mag_reconstructed, [tone_peak-1, tone_peak, tone_peak+1]).sum()
    ...
    """axs[1].plot(adc_reference, 'r--')
    axs[1].plot(adc_values_interfered, 'g-')
    axs[1].plot(adc_interferences_zeroed, 'b-')
    axs[1].plot(adc_interfered_reconstructed, 'black')
    plt.show()"""
