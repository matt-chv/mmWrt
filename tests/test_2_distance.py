...
from mmWrt.Scene import two_way_range  # scene_distance
...
def test_distance_5p1m_t_1000(time):
    ...
    # note since we do compute 2 ways (from antenna to scatterer and back to antenna), 5.1*2
...
def test_distance_5p1m_times():
    ...
    d0 = two_way_range(antenna_positions, scatterers_positions, antenna_positions)
...
def test_distance_multiple_antennas():
    ...
    antenna_positions[:, :, :] = [a.position_in_time(t=times) for a in antennas_ULA_64_60G[0:8]]
