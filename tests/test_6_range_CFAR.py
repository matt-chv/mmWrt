...
from mmWrt.RadarSignalProcessing import cfar_ca, cfar_alpha, \
    ERR_CFAR_CELL_COUNT
...
def test_cfar_length():
    ...
    try:
        _ = cfar_ca(fake_fft, guard_cell_count=1,
...
def test_rsp_cfar_length(mag_length):
    ...
    threshold = cfar_ca(fake_fft, guard_cell_count=1,
                        train_cell_count=2, pfa=0.01)
    assert threshold.shape == fake_fft.shape, \
        "CFAR threshold should have same length as input"
...
def test_rsp_cfar_ex1():
    ...
    fft_mag = np.abs(fft)
    threshold = cfar_ca(fft_mag, guard_cell_count=1,
                        train_cell_count=2, pfa=0.01)
...
def test_rsp_cfar_ex3(peak_idx: int):
    ...
    assert np.where((magnitude > threshold + 1e-10))[0].size > 0, \
        f"no peaks detected but should have been at idx: {peak_idx}"
    if np.where((magnitude > threshold + 1e-10))[0].size > 0:
        assert peak_idx == np.where((magnitude > threshold + 1e-10))[0][0], \
            f"peaks detected at {np.where((magnitude > threshold + 1e-10))[0][0]} but should have been at idx: {peak_idx}"  # noqa 501
...
def test_rsp_grouping_ex5():
    ...
    assert np.allclose(grouped_idx, np.array([1, 9])), \
        f"Expected peak at index 1 and 9, got {grouped_idx}"
...
def tbd_test_FMCW_1j():
    ...
    Distances, range_profile = rsp.range_fft(adc_cube, bb)
...
def tbd_test_FMCW_radar_equation():
    ...
    scatterer2 = Scatterer(10, 0, 0, xt=lambda t: 2*t+10,
                           rcs_f=lambda f: 43000)
    scatterers = [scatterer1, scatterer2]

    bb = rt_points([radar], scatterers, radar,
                   radar_equation=True, debug=False)
    adc_cube = bb['adc_cube'][0][0][0]
    Distances, range_profile = rsp.range_fft(adc_cube, bb)
    ca_cfar = rsp.cfar_ca_1d(range_profile)
...
def tbd_test_FMCW_real_adc_po2():
    ...
    bb = rt_points([radar], scatterers, radar, debug=False)
    data_matrix = bb['adc_cube'][0][0][0]
    Distances, range_profile = rsp.range_fft(data_matrix, bb)
    ca_cfar = rsp.cfar_ca_1d(range_profile)
    ...
    try:
        _ = Radar(transmitter=Transmitter(bw=3.5e9, slope=70e8),
                  receiver=Receiver(adc_sample_rate=1e4,
                                    adc_sample_rate_max=1e5,
                                    adc_sample_count_max=2048*4,
                                    adc_sample_count=2048*3,
                                    debug=True))
    except ValueError as ex:
        str_ex = str(ex)
...
def tbd_test_Nyquist():
    radar = Radar(transmitter=Transmitter(bw=3.5e9, slope=70e8),
                  receiver=Receiver(adc_sample_rate=3e3,
                                    adc_sample_count_max=2048),
                  debug=False)
    ...
    str_ex = ""
    try:
        _ = rt_points([radar], scatterers, radar,
                      debug=True)
    except ValueError as ex:
        str_ex = str(ex)
...
def test_high_speed():
    ...
    TODO: need to clean the code to make it more legible
    """
    ...
    Tc = NA*1/fs0
    # The scatterer distance grows 2x (there and back)
    # so speed needs to be divided by 2
    ...
    assert speed_estimate == known_result_from_estimate, \
        f"speed of scatterer is: {v1} close to: {known_result_from_estimate}, \
         but found to be: {speed_estimate}"
