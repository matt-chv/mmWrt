        ...
        for frame_i in range(n_frames):
            for chirp_i in range(n_chirps):
                for tx_i in range(n_tx):
                    ...
                    if "interferers" in raytracing_opt:
                        # duplicate for own TX and interferers TXs
                        for interferer in raytracing_opt["interferers"]:
                            for interferer_tx in interferer:
                                f_tx = interferer_tx.TX_freq(T)
                                if f_tx:
